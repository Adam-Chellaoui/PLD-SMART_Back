...
export const authenticateEventOwner = async(connection, req, res, next) => {
    ...
    if(!eventId)
        return res.status(400).json({error: "Bad or empty eventId"})

    //Have to check that the user authenticated is the event creator
    const [results, fields] = await connection.execute("SELECT creator_id FROM Event WHERE id = ?", [eventId]) 
    ...
    if(creator_id !== req.userId){
        return res.status(401).json({error: "User is not authorized to this event."})
    }
