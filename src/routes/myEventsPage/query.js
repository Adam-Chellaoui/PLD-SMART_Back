const comingEventsQuery = () => {
    const req = `SELECT event.name, event.photo as ImageEvent, event.date_timestamp, event.place, creator.photo as ImageProfil, event.creator_id as CreatorId, event.id as event_id  FROM eve.Event as event, eve.User as creator, eve.Participation as p  where event.date_timestamp>=Now() and creator.id=event.creator_id and event.id=p.event_id and p.user_id=? and event.status_id=1 order by event.date_timestamp`
    return req
};
