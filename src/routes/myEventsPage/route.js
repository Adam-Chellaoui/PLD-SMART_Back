...
const getComingEventsRoute = async (connection, req, res) => {
  ...
  try {
    const [results, fields] = await connection.execute(comingEventsQuery(), [
      id,
    ]);
...
const getMyHistoric = async (connection, req, res) => {
  ...
  try {
    const [results, fields] = await connection.execute(myHistoric(), [id]);
    res.status(200).send(results);
  } catch (err) {
    console.log(err);
    res.status(500).json({ message: "An error ocurred: " });
...
const getMyFavorite = async (connection, req, res) => {
  ...
  try {
    const [results, fields] = await connection.execute(myFavorite(), [id]);
    res.status(200).send(results);
  } catch (err) {
    console.log(err);
    res.status(500).json({ message: "An error ocurred: " });
