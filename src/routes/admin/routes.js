...
const adminDeleteEvent = async (connection, req, res) => {
  ...
  if (!eventId) res.status(400).json({ error: "Param eventId empty." });
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "An error occured. Check if the eventId exists." });
  }
};

const adminBlockUser = async (connection, req, res) => {
  const { targetUserId } = req.body;
  if (!targetUserId) res.status(400).json({ error: "Empty userId." });

  //Check if user id exists
  try {
    const [results, fields] = await connection.execute(checkUserExistsQuery(), [
      targetUserId,
    ]);
    if (!results)
  ...
  try {
    const [results, fields] = await connection.execute(adminBlockUserQuery(), [
      targetUserId,
    ]);
