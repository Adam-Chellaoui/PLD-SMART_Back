...
const editInfoUserRoute = async (connection, req, res) => {
    ...
    try{
        const [results, fields] = await connection.execute(editInfoUser(), [phone, city, streetNumber, street,  region,  zipCode, adressComplement, gender, dateBirth,  userPassword, id])
...
const editImageProfilRoute = async (connection, req, res) => {
    ...
    try{
        const [results, fields] = await connection.execute(editImageUser(), [photo, id])
        res.status(200).send({message: "Success "});
    }catch(err){
        console.log(err)
        res.status(500).json({message: "An error ocurred: "})
...
const createReportRoute = async (connection, req,res)=>{
    ...
    try{
        const [results, fields] = await connection.execute(createReport(), [user_id, type_id])
        res.status(200).send({message: "Success "});
    }catch(err){
        console.log(err)
        res.status(500).json({message: "An error ocurred: "})
